...
- **TurboVec** ✅ (`github.com/RyanCodrai/turbovec`, MIT, Rust+PyO3, TurboQuant 16×) — the vector index, allowlist filtered search. Persist the index; load on cold start (≈384 B/vec keeps it small); revisit if cold-start latency hurts.
  ...
  - **BUILT vs WIRED (honest status):** P0.2b delivers the TurboVec index, the **allowlist permission pre-filter** (`TurboVecIndex.search(allowlist_chunk_ids=...)`), `governance.allowlist.authorized_chunk_ids`, and the offline build — **all unit-proven** (drop-before-search excludes unauthorized vectors at the index; pre-filter == post-filter). They are **NOT yet wired into live retrieval**: `HybridRetriever` still uses the permission-blind in-memory store + the L5 post-filter, and the allowlist/`search(allowlist=)` have **no production callers yet**. Live vector-path enforcement + the mandatory-reranker wiring + the end-to-end C1-over-vector test (class-D chunk via TurboVec → `[REDACTED]` in answer + citations) are the **retrieval-integration phase (P0.2c)** — see the decomposition. Do not claim live vector-path enforcement until P0.2c lands.
