...
- **TurboVec** ✅ (`github.com/RyanCodrai/turbovec`, MIT, Rust+PyO3, TurboQuant 16×) — the vector index, allowlist filtered search. Persist the index; load on cold start (≈384 B/vec keeps it small); revisit if cold-start latency hurts.
  - **The reranker is LOAD-BEARING, not optional (P0.2b finding).** Measured TurboVec coarse recall@10 vs exact cosine (bit_width=4): **~0.45 on tightly-clustered embeddings** (4-bit can't finely rank *within* a ~100-vector near-tie cluster — it returns the right *neighborhood*), **~0.90+ on real-text topical clusters**. A full-precision rerank over the coarse candidate set recovers exact top-10 to **~0.80+**. Therefore P0.2b+ retrieval **must always rerank the TurboVec candidate set** (over-fetch coarse → cross-encoder rerank → final_top_k); disabling rerank would degrade precision on clustered data. The S1 random-vector recall (~0.85) was not representative — this is the honest, real-embedding characterization.
  - **WIRED + LIVE (P0.2c, done):** the TurboVec index + allowlist pre-filter are now **wired into live retrieval** via `TurboVecRetriever.retrieve_for_session(query, session)`: per-session allowlist (governance) → `TurboVecIndex.search(allowlist=...)` index pre-filter → store-hydrate → **mandatory rerank** (non-disableable on the vector path) → L5 C1 masking. The pipeline routes vector-mode queries through it (the permission-blind path is kept for the INTERN-vs-CFO leak demo). **Live enforcement proven** by the end-to-end C1-over-vector test (`tests/test_c1_over_vector_e2e.py`): a class-D chunk via the live TurboVec path is `[REDACTED]` in answer + citations with raw PII absent, and a class-F (L5) chunk is never retrieved for an L2 analyst (allowlist pre-filter, live) while the CFO does receive it. (P0.2b delivered + unit-proved these pieces; P0.2c made them live.)
