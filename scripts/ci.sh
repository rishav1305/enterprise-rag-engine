...
SKIPS=$(PYTHONPATH=src "$PYTHON" -m pytest -m "not cloud and not bq and not llm and not langfuse and not spicedb and not oso" -rs -q \
    ...
    tests/test_api_metadata_endpoints.py \
    tests/test_corpus_mask_partial.py tests/test_corpus_governance_hardening.py \
    tests/test_trace_parity.py tests/test_trace_endpoint.py \
    tests/test_query_redaction_regression.py \
